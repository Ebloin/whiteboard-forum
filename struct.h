...
//Check if a user exists from the user index
static int check_utente_exists(whiteboard* w, int userindex) {
...
//Check if a message has been visualized from all users
static int check_visualized(message* m) {
	int i;
...
//Recoursive functionthat prints a message replies indented of one tab
static void print_replies(message* ml, message* m, char* buf, int topic_index, int level, int userindex) {
	int i=0,l=0;
	...
	for (i=0; i<NUM_MESSAGES; i++) {
		if (m->replies[i] != 0) {
			...
			sprintf(tmp, "\033[1;34m[#%s]\033[0m %s", current->id, current->text);
			strcat(buf, tmp);
...
static int check_subscription(topic* t, int userindex) {
	int i;
	for (i=0; i<NUM_USERS; i++) {
...
static void list_subscripted_topics(whiteboard* w, topic* tl, int userindex, char* buf) {
...
static void list_messages_from_topic(whiteboard* w, char* buf, int topic_index, int userindex) {
...
// -2 username not correct
// -3 password not correct
...
// -2 no existent user
// 0 ok
static int delete_user(whiteboard* w, int u) {
	if (u == 0) {
		//cannot delete admin
		return -1;
	}
...
static int subscribe_to_topic(whiteboard* w, int topicindex, int userindex) {
	int i;
...
//Custom function to send data over the socket, pad the sent string to 1024 bytes
...
static void get_topic_index(int* topicindex, char* topic_id) {
	char topic[5] = {0};
...
static int print_status_message(whiteboard* w, int topic_index, int msg_index, char* buf, int requester) {
	...
	for (i=1; i<NUM_USERS; i++) {
		if (check_utente_exists(w, i)) {
			if (m->visualizations[i] == 1) {
				printf("entro0\n");
				sprintf(tmp, "\t\033[1;34m%s\033[0m", w->users[i].username);
				strcat(buf, tmp);
			}
		}
	}
